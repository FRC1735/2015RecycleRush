   ...
   // Wrapper function for handling ratchet plus lifter.   Called by Commands.
    ...
    public void liftWithJoystick(Joystick lifterJoy, double currentTime) {
        ...
        if (magnitude >= 0) {
			...
			if (RobotMap.lifterRatchetServo.getAngle() >1) { // engaged is 90.
				//Then we are requesting a state change to the ratchet.
				//Set a 40ms timer to let server get out of the way before we engage the motor.
				m_ratchetWaitTime = currentTime + 0.4;// 20ms polling loop, this should put us in the next loop.
			}
			ratchetDisengage();
	    }
